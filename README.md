...
2. [Архитектура решения](#-2архитектура-решения)
3. [Подготовка окружения](#%EF%B8%8F-3-подготовка-окружения)
    - [Подготовка облака](#подготовка-облака)
    - [Создание сервисных аккаунтов и API-ключа](#создание-сервисных-аккаунтов-и-api-ключа)
4. [Развёртывание инфраструктуры](#%EF%B8%8F-4-развёртывание-инфраструктуры)
    - [Создание адреса Postbox](#создание-адреса-postbox)
    - [Создание MCP-сервера wordstat](#создание-mcp-сервера-wordstat)
    - [Создание агента-исследователя research-agent](#создание-агента-исследователя-research-agent)
    - [Создание агента-валидатора validator-agent](#создание-агента-валидатора-validator-agent)
    - [Создание агента-писателя writer-agent](#создание-агента-писателя-writer-agent)
    - [Создание рабочего процесса Workflows](#создание-рабочего-процесса-workflows)
    - [Создание MCP-сервера article-workflow](#создание-mcp-сервера-article-workflow)
    - [Создание агента-орекстратора brief-agent](#создание-агента-оркестратора-brief-agent)
    - [Подключение AI-агента к Open WebUI](#подключение-ai-агента-к-open-webui)
5. [Тестирование решения](#-5-тестирование-решения)
6. [Очистка ресурсов](#-6-очистка-ресурсов)
...
5. **Доставка:** Готовый текст автоматически отправляется на вашу почту. В ответе Агента-орекстратора вы получаете краткую сводку: какие тренды были найдены, какой из них признан самым горячим, и уведомление о том, что готовая статья уже ждет вас на e-mail.
...
| Инструмент Web Search              | Встроенный инструмент AI Studio для поиска информации в интернет.                                                                                        |
| validator-agent AI Studio агент | Агент-валидатор подключается к данным Wordstat и выбирает из тройки лидеров самый популярный запрос, гарантируя высокий интерес аудитории. |
| wordstat MCP-сервер | MCP-сервер определяет популярные тренды на основе статистики поисковых запросов к Яндексу с помощью инструмента Wordstat. |
| Инструмент Wordstat | Для определения популярных трендов на основе статистики поисковых запросов к Яндексу. |
...
## Создание сервисных аккаунтов и API-ключа
...
[Создайте MCP-сервер](https://aistudio.yandex.ru/docs/ru/ai-studio/operations/mcp-servers/create-brand-new.html) для определения популярных трендов на основе статистики поисковых запросов к Яндексу с помощью [инструмента Wordstat](https://aistudio.yandex.ru/docs/ru/search-api/concepts/wordstat.html):
